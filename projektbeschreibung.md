...
const exportOrders = () => {
  ...
  // ... Excel-Blob-Erstellung ...
};
```

### 6. Benutzerverwaltung & Login

Benutzer werden in der MySQL-Datenbank gespeichert. Login erfolgt über Username und Passwort, die per bcrypt gehasht sind.

```typescript
// Backend: User anlegen
await connection.execute('INSERT INTO users (name, username, password, role) VALUES (?, ?, ?, ?)', ['eileen', 'eileen', hash, 'user']);
...
## Beispiel: Login-Testbenutzer

```md
Mitarbeiter: gio / gio123
             claudio / claudio123
             gerrik / gerrik123
             martin / martin123
             julia / julia123
             eileen / eileen123
Admin:       cristian / cristian123
             debby / debby123
```

## Besonderheiten
- Kategorien und Produktnamen sind flexibel und können angepasst werden (z.B. "Small Friends" statt "Small Buds", "420Mix" statt "Trim")
- Mobile-optimierte Tabellen und UI
- Tageslogik für fertige Aufträge
- Export mit UTF-8 BOM für Excel-Kompatibilität

## Weiterentwicklung
- Erweiterung der Analyse-Funktionen
- Automatisierte Lagerwarnungen
- Benutzerrollen und Rechte granularer gestalten
